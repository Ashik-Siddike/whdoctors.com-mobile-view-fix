...
<div class="container py-4">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header table-header d-flex justify-content-between align-items-center">
            <div>
                <div class="table-title h5 mb-1 fw-semibold">Edit Amount</div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 small">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('amount.index') }}">Amount</a>
                        </li>
            ...
            <form action="{{ route('amount.update', $amount->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="agent_id" class="form-select" required>
                            ...
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}" {{ old('agent_id', $amount->agent_id) == $agent->id ? 'selected' : '' }}>
                                    {{ $agent->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="date" class="form-control" value="{{ old('date', $amount->date->format('Y-m-d')) }}" required>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="currency" class="form-select" required>
                            @foreach(['BDT','USD','INR','PKR'] as $currency)
                                <option value="{{ $currency }}" {{ old('currency', $amount->currency) == $currency ? 'selected' : '' }}>
                                    {{ $currency }}
                                </option>
                            @endforeach
                        </select>
