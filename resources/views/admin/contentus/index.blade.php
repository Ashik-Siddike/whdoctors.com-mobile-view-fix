                ...
                <div class="card table-card">
                    <div class="card-header table-header">
                        <div class="title-with-breadcrumb">
                            <div class="table-title">content Section</div>
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb mb-0">
                                    <li class="breadcrumb-item">
                                        <a href="{{route('dashboard')}}">Dashboard</a>
                                    </li>
                                    <li class="breadcrumb-item active" aria-current="page">content Section</li>
                                </ol>
                            </nav>
                        </div>
                        <a href="{{route('contentus.create')}}" class="add-new">Add content Section<i class="ms-1 ri-add-line"></i></a>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach($content as $i => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('contentus.show', $item->id) }}" class="btn btn-sm btn-primary me-2"><i class="fa fa-eye"></i></a>
                                                <a href="{{ route('contentus.edit', $item->id) }}" class="btn btn-sm btn-warning me-2"><i class="fa fa-edit"></i></a>
                                                <form action="{{ route('contentus.destroy', $item->id) }}" method="post" class="d-inline delete-form" data-id="{{ $item->id }}">
                                                    @csrf
                                                    @method('DELETE')
