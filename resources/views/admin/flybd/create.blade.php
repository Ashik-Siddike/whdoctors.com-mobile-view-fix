                ...
                <div class="card table-card pb-5">
                    <div class="card-header table-header">
                        <div class="title-with-breadcrumb">
                            <div class="table-title">Flybd</div>
                    ...
                    <div class="card-body custom-form">
                        <form action="{{ route('flybds.store') }}" method="POST" class="row g-3 mt-0">
                            @csrf
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <select class="form-control custom-input single-select2" name="flybd_category_id" required>
                                            <option value="" disabled selected> -- Select Flybd --</option>
                                            @foreach ($flybds as $flybd)
                                                <option value="{{ $flybd->id }}" {{ old('flybd_id') == $flybd->id ? 'selected' : '' }}>
                                                    {{ $flybd->title }}  {{-- এখানে Flybd এর title দেখাবে --}}
                                                </option>
                                            @endforeach
                                        </select>
